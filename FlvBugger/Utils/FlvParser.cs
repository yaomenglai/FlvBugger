                    ...
                    else if (tag is AudioTag)
                    {
                        ...
                        if (10 == codec)
                        {
                            ...
                            if (buffer[0] == 0)
                            {
                                ...
                                if (rtn <= 0)
                                    return null;
                                AudioTag audo = (AudioTag)tag;
                                audo.buffer = new byte[2];
                                audo.buffer[0] = buffer[0];
                                audo.buffer[1] = buffer[1];
                                stream.Seek(-3, SeekOrigin.Current);
                            }
                            //stream.Seek(tag.DataSize - 1, SeekOrigin.Current);
                            stream.Seek(offset + tag.DataSize - 1, SeekOrigin.Begin);
                        }
                        else
                        {
                            stream.Seek(tag.DataSize - 1, SeekOrigin.Current);
        ...
        public class AudioTag : FlvTag
        {
            ...
            public string Config
            {
                get
                {
                    if (null == buffer)
                        return "";
                    int audioObjectType = (buffer[0] & 0xF8) >> 3;//< -取前5bit，结果为2
                    int samplingFrequencyIndex = ((buffer[0] & 0x7) << 1) | (0x90 >> 7);// < -取后4bit，结果为3
                    int channelConfiguration = (buffer[1] >> 3) & 0x0F;// < -取后4bit，结果为2
                    int frameLengthFlag = (buffer[1] >> 2) & 0x01;// < -取后1bit，结果为0
                    int dependsOnCoreCoder = (buffer[1] >> 1);// & 0x01 < -取后1bit，结果为0
                    int extensionFlag = buffer[1] & 0x01;// < -最后1bit，结果始终为0
                    String ret = "";
                    String sample = "";
                    switch (samplingFrequencyIndex)
                    {
                        case 0:
                            sample = "96000 Hz"; break;
                        case 1:
                            sample = "88200 Hz"; break;
                        case 2:
                            sample = "64000 Hz"; break;
                        case 3:
                            sample = "48000 Hz"; break;
                        case 4:
                            sample = "44100 Hz"; break;
                        case 5:
                            sample = "32000 Hz"; break;
                        case 6:
                            sample = "24000 Hz"; break;
                        case 7:
                            sample = "22050 Hz"; break;
                        case 8:
                            sample = "16000 Hz"; break;
                        case 9:
                            sample = "12000 Hz"; break;
                        case 10:
                            sample = "11025 Hz"; break;
                        case 11:
                            sample = "8000 Hz"; break;
                        case 12:
                            sample = "7350 Hz"; break;
                        default:
                            sample = "Reserved"; break;
                    }
                    ret += "config sample:" + sample;
                    ret += "config channel:" + channelConfiguration;
                    return ret;
                }
            }
